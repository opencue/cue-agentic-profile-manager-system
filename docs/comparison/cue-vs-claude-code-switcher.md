...
**Short answer:** `claude-code-switcher` swaps between named MCP configurations — useful if you keep separate sets of MCP servers for different projects. [cue](https://github.com/recodeee/cue) does that **plus** skills, plugins, rules, commands, hooks, persona, playbooks, quality gates, and evals — a full per-directory agent loadout, not just an MCP set.
