...
**Short answer:** [skillport](https://github.com/search?q=skillport) is a skill installer — it pulls SKILL.md files from a registry into your `~/.claude/skills/` directory. [cue](https://github.com/recodeee/cue) is an agent profile manager — it scopes skills + MCPs + plugins + persona + playbooks + quality gates per-directory, with inheritance, materialized isolation, and a failure-feedback loop.
