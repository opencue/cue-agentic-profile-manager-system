# core

Core is the baseline inherited by the first-install profiles. It keeps the always-on surface small: repo/file reading, skill lookup, quick-task routing, house-style brevity, and Lore-compatible commit help.

## Skills

- `meta/analyze`
- `meta/just`
- `meta/skill-suggestion`
- `caveman/caveman`
- `caveman/caveman-commit`

## MCPs

- `claude-mem`

## Sample Tasks

- Read a repo and summarize the highest-risk files.
- Pick the right local skill for a terse user request.
- Write a Lore-style commit message after a small change.
